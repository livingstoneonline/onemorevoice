...
<!-- Useful for DTDs https://www.w3.org/TR/xhtml1/dtds.html#h-A2 -->
	...
	<xsl:template match="/">
		...
		<html lang="en">
			...
			<head>
				...
				<!-- CSS lazy load adapted from https://blog.webjeda.com/lazy-load-css/ -->
	...
	<xsl:template match="figure">
		...
		<xsl:variable name="rotate-id">
			<xsl:if test="self::node()[@n='artifact rotate-180']">
		...
		<!-- end of variables -->
		<xsl:choose>
			<xsl:when test="contains(@n,'artifact') and contains(@n,'rotate-180')">
